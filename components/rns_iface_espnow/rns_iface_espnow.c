...
#define RNS_ESPNOW_MAGIC_0 0x4d
#define RNS_ESPNOW_MAGIC_1 0x45
#define RNS_ESPNOW_FRAME_VERSION 0x01
                                           ...
                                           size_t *fragment_count)
{
    ...
    if (payload_max == 0 || payload_max > RNS_ESPNOW_MAX_FRAGMENT_PAYLOAD) {
        return ESP_ERR_INVALID_SIZE;
    }

    size_t count = (packet_len + payload_max - 1) / payload_max;
    if (count == 0 || count > RNS_ESPNOW_MAX_FRAGMENTS || max_fragments < count) {
        return ESP_ERR_INVALID_SIZE;
    }
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        memcpy(fragments[i].message_id, full_hash, RNS_ESPNOW_MESSAGE_ID_SIZE);
                                         ...
                                         size_t *written)
{
    ...
    if (fragment->count == 0 ||
        fragment->count > RNS_ESPNOW_MAX_FRAGMENTS ||
        fragment->index >= fragment->count ||
        fragment->payload_len > RNS_ESPNOW_MAX_FRAGMENT_PAYLOAD) {
        return ESP_ERR_INVALID_ARG;
    }

    size_t needed = RNS_ESPNOW_FRAGMENT_HEADER_SIZE + fragment->payload_len;
    if (frame_len < needed || needed > RNS_ESPNOW_MAX_FRAME_SIZE) {
        return ESP_ERR_INVALID_SIZE;
    }

    size_t pos = 0;
    frame[pos++] = RNS_ESPNOW_MAGIC_0;
    frame[pos++] = RNS_ESPNOW_MAGIC_1;
    frame[pos++] = RNS_ESPNOW_FRAME_VERSION;
    ...
    memcpy(frame + pos, fragment->message_id, RNS_ESPNOW_MESSAGE_ID_SIZE);
    pos += RNS_ESPNOW_MESSAGE_ID_SIZE;
                                           ...
                                           rns_espnow_fragment_t *fragment)
{
    ...
    if (frame_len < RNS_ESPNOW_FRAGMENT_HEADER_SIZE ||
        frame_len > RNS_ESPNOW_MAX_FRAME_SIZE ||
        frame[0] != RNS_ESPNOW_MAGIC_0 ||
        frame[1] != RNS_ESPNOW_MAGIC_1 ||
        frame[2] != RNS_ESPNOW_FRAME_VERSION) {
        return ESP_ERR_INVALID_SIZE;
    }
    ...
    if (fragment->count == 0 ||
        ...
        fragment->index >= fragment->count ||
        fragment->payload_len > RNS_ESPNOW_MAX_FRAGMENT_PAYLOAD ||
        frame_len != RNS_ESPNOW_FRAGMENT_HEADER_SIZE + fragment->payload_len) {
        return ESP_ERR_INVALID_SIZE;
    }

    memcpy(fragment->message_id, frame + pos, RNS_ESPNOW_MESSAGE_ID_SIZE);
    pos += RNS_ESPNOW_MESSAGE_ID_SIZE;
...
static bool same_message(const rns_espnow_reassembler_t *reassembler,
                         const rns_espnow_fragment_t *fragment)
{
...
static void start_message(rns_espnow_reassembler_t *reassembler,
                          const rns_espnow_fragment_t *fragment)
{
    memset(reassembler, 0, sizeof(*reassembler));
    reassembler->active = true;
    reassembler->expected_count = fragment->count;
    memcpy(reassembler->message_id, fragment->message_id, RNS_ESPNOW_MESSAGE_ID_SIZE);
}
                                              ...
                                              bool *complete)
{
    ...
    *complete = false;
    ...
    rns_espnow_fragment_t fragment;
    esp_err_t err = rns_iface_espnow_unpack_fragment(frame, frame_len, &fragment);
    if (err != ESP_OK) {
        return err;
    }
