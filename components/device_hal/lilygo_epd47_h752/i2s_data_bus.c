...
#define H752_I80_LINE_BYTES ((960 + 32) / 4)
#define H752_I80_DMA_ALIGN  64
...
{
    /*
     * The esp_lcd I80 path uses a single DMA line buffer. tx_color() queues an
     * asynchronous transfer, so the caller must not render the next row into
     * this buffer until the transfer-complete callback has fired.
     */
    while (i2s_is_busy()) ;
}
#endif
...
void IRAM_ATTR i2s_start_line_output()
{
    ...
    esp_lcd_panel_io_tx_color(io_handle, 0, buffer, H752_I80_LINE_BYTES);
}
#endif
...
{
    if (buffer == NULL) {
        buffer = heap_caps_aligned_alloc(H752_I80_DMA_ALIGN,
                                         H752_I80_LINE_BYTES,
                                         MALLOC_CAP_INTERNAL | MALLOC_CAP_DMA);
        ESP_ERROR_CHECK(buffer ? ESP_OK : ESP_ERR_NO_MEM);
        memset(buffer, 0, H752_I80_LINE_BYTES);
    }
    ...
    // start_pulse_pin = cfg->start_pulse;
        ...
        .bus_width = 8,
        .max_transfer_bytes = H752_I80_LINE_BYTES
    };
